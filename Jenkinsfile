            ...
            steps{
                script{
                    withCredentials([string(credentialsId: 'docker_pass', variable: 'docker_password')]) {
                             sh '''
                                docker build -t 35.207.23.98:8083/springapp:${VERSION} .
                                docker login -u admin -p $docker_password 35.207.23.98:8083 
                                docker push  35.207.23.98:8083/springapp:${VERSION}
                                docker rmi 35.207.23.98:8083/springapp:${VERSION}
                            '''
                    }
                }
            }
                    ...
                    withCredentials([string(credentialsId: 'docker_pass', variable: 'docker_password')]) {
                          dir('kubernetes/') {
                             sh '''
                                 ...
                                 curl -u admin:$docker_password http://35.207.23.98:8081/repository/helm-hosted/ --upload-file myapp-${helmversion}.tgz -v
                            '''
                          }
                    }
                }
